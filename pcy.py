...
def pcy(baskets, support, bucket_size):
    ...
    while 1:
        ans_one = pcy_pass_one(baskets, candidates, support, bucket_size)
        ans_two = pcy_pass_two(baskets, ans_one, bucket_size)
...
def pcy_pass_one(baskets, candidates, support, bucket_size):
    print("Len baskets = ", len(baskets))
    result = []
    itemset_size = 1
    if len(candidates) > 0:
    ...
    frequent_items = generate_frequent_items(item_counts, len(baskets)*support)
    ...
    bitmap = generate_bitmap(bucket, len(baskets)*support)
    bitmap_list = bitmap_to_list(bitmap, bucket_size)
    result.append(bitmap)
    print("PASS one result", result)
    return result


def pcy_pass_two(baskets, ans_one, bucket_size):
    result = []
